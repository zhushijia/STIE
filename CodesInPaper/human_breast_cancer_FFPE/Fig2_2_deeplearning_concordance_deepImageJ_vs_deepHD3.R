...
###########################################################
######### STIE result
#3.5 is the best, but missed endothelial, to match DL, we choose 2.5
############################################################
result = results[['2.5']]
...
celltypes = c("Plasmablasts", "Bcells", "Tcells", "Myeloid", 
              "Endothelial", "CAFs", "PVL", 
              "NormalEpithelial", "CancerEpithelial")
Signature = Signature[, match(celltypes,colnames(Signature))]
...
#write.table(table2df(t2),"DL_STIE_concordance_larger_category_full_signature.txt",sep="\t",col.names=T,row.names=F,quote=F)
...
cc = do.call(cbind, lapply(1:ncol(t3),function(i) {
    ...
    ci = cor( Signature, as.numeric(merged_expr[,i]), method='pearson' )
...
setwd("/archive/SCCC/Hoshida_lab/shared/fastq/SpatialTranscriptome/10X_public_dataset/HumanBreastCancer_FFPE/count/results/DL")
#write.table(cc,"DL_STIE_scRNA_Signature_concordance_larger_category_full_signature.txt",sep="\t",col.names=T,row.names=F,quote=F)
...
setwd("/archive/SCCC/Hoshida_lab/shared/fastq/SpatialTranscriptome/10X_public_dataset/HumanBreastCancer_FFPE/count/results/DL")
#write.table(c2,"DL_STIE_scRNA_Signature_concordance_larger_category_full_signature_allinfo.txt",sep="\t",col.names=T,row.names=F,quote=F)
