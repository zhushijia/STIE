...
cells_on_spot = STIE_result[['2.5']]$cells_on_spot
cell_types = STIE_result[['2.5']]$cell_types

if(1)
{
    Signature = STIE_result[['2.5']]$Signature
...
prop = with(STIE_result[['2.5']], table(as.character(cells_on_spot$spot), cell_types) )
prop = t(apply(prop,1,function(x)x/sum(x)))
#selected_spots = sample( unique(as.character(cells_on_spot$spot)), num )
selected_spots = unique(do.call(c, lapply(1:ncol(prop), function(i) {
    ind = order(prop[,i],decreasing=T)
    rownames(prop)[ ind[1:num] ]
} )))
...
cor(STIE_result[['2.5']]$Signature, Signature)
...
table(STIE_result[['2.5']]$cell_types, result$cell_types)
