...
## run deconvolution
############################################################
STIE_result = STIE(ST_expr, Signature, cells_on_spot, features=c('size','shape'),
                steps=30, known_signature=TRUE, known_cell_types=FALSE, 
                lambda=1e3, min_cells=-1)
...
cc = data.frame(celltypes = c("Plasmablasts","Bcells", "Tcells", "Myeloid", 
                ...
                colors = c( "#4DAF4A", "darkorange", "steelblue", "yellow", 
                            "cyan", "darkred", "purple",
                            "black" ) )
cc = subset(cc, celltypes%in%as.character(STIE_result$cell_types) )
