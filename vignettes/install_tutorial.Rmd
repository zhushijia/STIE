...
```{r}
First, we need to export two variables to let R know the lib of ImageMagick (have not got a chance to test if both are necessary):
export PATH=$PATH:XXX/ImageMagick-7.1.0/lib/pkgconfig:XXX/ImageMagick-7.1.0/lib
export PKG_CONFIG_PATH=XXX/ImageMagick-7.1.0/lib/pkgconfig:XXX/ImageMagick-7.1.0/lib
 
Even with those specified, I still met with two errors:
1. there is no "Magick::ReadOptions.quiet" in Magick++;
2. cannot locate libMagick++-6.Q16HDRI.so.5;
 
#1: I am guessing, it is because of the incompatibility between the latest magick ver2.7 in R (automatically installed by "install.packages") and imageMagick (ver 6.8 or 6.9) on bioHPC. So, I upgraded imageMagick to ver 7.1 via installing it locally.
 
#2: given #1, I can locate libMagick++-7.Q16HDRI.so.5 from my own installation.
To make R know it, I need to export another variable LD_LIBRARY_PATH as following:
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:XXX/ImageMagick-7.1.0/lib
