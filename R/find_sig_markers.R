#' find_sig_markers
#'
#' find_sig_markers estimates the cell type gene markers from spot level gene expression and STIE estimation
#'
...
#' @param transform a character value representing the method to transform the count data
#' @param DEG_pthres a numeric value representing the pvalue threshold for DEG calling
#'
#' @return a data frame for cell type marker genes with columns of features, diff and pvalue 
#' @export
#' 
...
#' 
#' 
find_sig_markers <- function(STIE_result, 
                             ST_expr, 
                             transform=c("logcount", "inverse.transform"), 
                             DEG_pthres=1e-5)
{
    ...
    features.info = do.call(rbind, lapply( 1:nrow(ST_expr3), function(i) {
        ...
        y = ST_expr3[i,]
        if(transform=="logcount") y = log(y+1)
