#' merge_feature2
#'
#' merge_feature2 merges the morphological features from all split images
#'
...
#' 
#' 
merge_feature2 <- function( feature_dir )
{
  ...
  contours <- do.call(c, lapply(1:length(feature_files), function(i) {
      ...
      nmask <- Image(as.matrix(fread(mask_files[i])))
      contour <- ocontour(nmask)
  ...
  voronois <- do.call(c, lapply(1:length(feature_files), function(i) {
      cat("extracting cell contour for image split:", i,"\n")
      nmask <- Image(as.matrix(fread(mask_files[i])))
      cmask <- propagate(nmask*0, seeds=nmask, lambda=100)
      contour <- ocontour(cmask)
  ...
  voronois = voronois[-duplicates]
  
  cell_id = 1:nrow(features)
  features =  data.frame(cell_id,features)
  names(contours) = cell_id
  names(voronois) = cell_id
  
  info <- list(cell_feature=features, cell_contour=contours, cell_voronoi=voronois)
